...
import {
  ...
  ContentSearchResultFacets,
} from "@/api-types";

async function handler(
  req: NextRequest
): Promise<NextResponse<SiteSearchResponse>> {
  const client = getSdk();
  const searchTerm = req.nextUrl.searchParams.get("term") ?? "";
  const contentTypes = (req.nextUrl.searchParams.get("types") ?? "")
    .split(",")
    .map(Utils.trim)
    .filter(Utils.isNonEmptyString);
  const contentLocales = (req.nextUrl.searchParams.get("locales") ?? "")
    .split(",")
    .map(Utils.trim)
    .filter(Utils.isNonEmptyString);
    ...
    return NextResponse.json(
      {
        error: {
          type: "Bad Request",
          message: "The term parameter is required",
        },
      },
      { status: 400 }
    );
  ...
  const rawResponse = await client.searchContent({
    ...
    topInterest,
    locale:
      contentLocales?.length > 0 ? (contentLocales as GraphQL.Locales[]) : null,
    types: contentTypes?.length > 0 ? contentTypes : null,
  });
  const resultItems: ContentSearchResultItems = (
    rawResponse._Content?.items ?? []
  )
  ...
  const contentTypeFacetInfo = (
    rawResponse._Content?.facets?._metadata?.types ?? []
  ).filter(Utils.isNotNullOrUndefined);
  resultFacets.push({
    field: "types",
    options: contentTypeFacetInfo
      .filter((x) => x.name != "_Content" && x.name != "Page")
      .map((x) => {
        return {
          value: x.name ?? "",
          count: x.count ?? 0,
  ...
  const languageFacetInfo = (
    rawResponse._Content?.facets?._metadata?.locale ?? []
  ).filter(Utils.isNotNullOrUndefined);
  resultFacets.push({
    field: "locales",
